import { obtEmpleado, obtEmpleadoTodo, obtEmpleadosActivo, obtTodo, registraEmpleado, desactivaEmpleado, activaEmpleado, modificaEmpleado, eliminaEmpleado, obtUsuarioPorEmail } from "../modelos/usuarioModelo.js";
...
export const registrarEmpleado = async (req, res) => {
    try {
        ...
        await check('password_hash').notEmpty().withMessage('El password no puede ir vacío').run(req);
        await check('password_hash').isLength({ min: 8 }).withMessage('Debe tener minimo 8 caracteres').run(req);
        await check('password_hash').matches(/[A-Z]/).withMessage('Debe tener al menos una mayuscula').run(req);
        await check('password_hash') .matches(/[a-z]/).withMessage('Debe tener al menos una minusculo').run(req);
        await check('password_hash').matches(/[0-9]/).withMessage('Debe tener al menos un numero').run(req);
        ...
        const hash = await bcrypt.hash(req.body.password_hash,  10);
